...
class Session {
  ...
  async changePassword(username, password, newPassword) {
    const identityUrl = await this.getIdentityUri();
    return this.transport.post(`${identityUrl}/pauth/passwords`, {
      body: JSON.stringify({
        username,
        password,
        newPassword
  ...
  startOnExpiryWarningTimer() {
    if (this.access && this.onExpiryWarning) {
      const expiryWarning =
        differenceInMilliseconds(this.access.expiresAt, new Date()) -
