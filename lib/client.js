 ...
 *
 * @param {string} directory - The directory to pass through to {@link Session}.
 * @param {string} registryUri - The registryUri to pass through to {@link Registry}.
 * @param {string?} identityUri - The identityUri to pass through to {@link Session}.
 *
...
class Client {
  ...
  login(username, password, options = { strategy: 'pauth' }) {
    return this.session.authenticateWithCredentials(username, password, options)
  ...
  generate(token) {
    ...
    return p.then(session => this.registry.fetch(session.currentOwner()))
      .then(() => this.session);
  }
  ...
  setTenancy(tenantName) {
    ...
    return this.registry.fetch(tenantName).then(() => this.session.populateAccess()).then(() => this.session);
  }
