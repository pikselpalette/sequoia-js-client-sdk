 ...
 * @param {Number?} expiryThreshold - The number of milliseconds before token expiry to trigger the onExpiryWarning callback. Default 60000.
 *
 ...
 */
class Session {
  constructor(transport, directory, registry, identityUri, expiryThreshold = 60000) {
    ...
    return this.populateTenants()
      ...
      .then(() => {
        if (this.access) {
          ...
          this.expiryWarningRef = delay(expiringAccess => this.tokenExpiring(expiringAccess), expiryWarning, this.access);
        }
        return this;
      });
  }
