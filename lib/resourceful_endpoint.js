 ...
 * from {@link Service#resourcefulEndpoint}
 *
...
class ResourcefulEndpoint {
   ...
   *
   * Useful for creating many resources for ingest (e.g. from parsing a CableLabs XML
   * file into Resources to save in one go).
   *
   * See the Tutorials section for in-depth ingest examples
   *
  ...
  /**
   * Get a stringified version of an {@link Resource} that is suitable
   ...
   * });
   *
   * contents.serialiseResource(resource);
   *
   * // Returns:
   * '{"contents":[{"ref":"test:testcontent","name":"testcontent","title":"A test resource"}]}'
   *
   * @deprecated since version 0.3.0. See {@link Resource#serialise}
   *
   * @returns {string}
   */
  ...
  serialiseResource(resource) {
    return JSON.stringify({
      [this.pluralName]: [resource.toJSON()]
    }, null, '  ');
  }
  ...
  all(criteria, options) {
    ...
    function accumulate(resourceCollection) {
      ...
      return resourceCollection.nextPage().then(c => accumulate(c))
        .catch((err) => {
    ...
    return this.browse(criteria, options)
      .then(c => accumulate(c))
   ...
   *
   * @deprecated since version 2.1.0. Use {@link ResourcefulEndpoint#readOne}
   *
   ...
   */
  read(ref, criteria, options) {
    // eslint-disable-next-line
    console.warn('ResourcefulEndpoint.read() is deprecated since 2.1.0.  Use ResourcefulEndpoint.readOne() instead.');
  ...
  store(resource, options) {
    return this.transport.post(this.endPointUrl(),
      Object.assign({ body: resource.serialise() }, options)).then(json => this.responseToResource(json));
  }
  ...
  update(resource, options) {
    return this.transport.put(this.endPointUrl(resource.ref), Object.assign({ body: resource.serialise() }, options))
  ...
  relationshipFor(relationshipName) {
    const [name] = (Object.entries(this.relationships)
                    .find(([, relationship]) => relationship.resourceType === relationshipName)
                    || [undefined]);
