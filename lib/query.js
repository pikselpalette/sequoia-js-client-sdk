...
class Query {
  ...
  addRelatedThroughFields(relationships = {}, allFields = []) {
    ...
    fields = filteredIncludes.length ? (fields.length ? fields : allFields).concat(filteredIncludes) : fields;

    parsedQueryString.fields = fields.length ? fields.join(',') : undefined;

    this.query = queryString.stringify(parsedQueryString, { encode: false, allowDots: true });
    return this;
  }
