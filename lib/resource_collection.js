      ...
      this.collection = this.rawData[this.resourcefulEndpoint.pluralName].map(
        (content) => {
          ...
          if (this.rawData.linked) {
            ...
            Object.keys(this.rawData.linked).forEach((relationshipName) => {
              const relationshipDescriptor = this.resourcefulEndpoint.relationships && this.resourcefulEndpoint.relationships[relationshipName]
                ? this.resourcefulEndpoint.relationships[relationshipName]
                : {};

              let linkByRefName = relationshipDescriptor.fieldNamePath;

              if (relationshipDescriptor.type === 'indirect') {
                const upperCasedLinkByRefName = relationshipDescriptor.filterName
                  ? relationshipDescriptor.filterName.replace('with', '')
                  : '';
                linkByRefName = upperCasedLinkByRefName.length
                  ? `${upperCasedLinkByRefName[0].toLowerCase()}${upperCasedLinkByRefName.slice(1)}`
                  : `${this.resourcefulEndpoint.singularName}Ref`;
              }

              resource.linked[relationshipName] = this.rawData.linked[relationshipName].filter((item) => {
                ...
                const { through, filterName, fieldNamePath } = relationshipDescriptor;

                // direct relationships
                if (relationshipDescriptor.type === 'direct' && resource[linkByRefName]) {
                  if (!fieldNamePath) return true;

                  return Array.isArray(resource[linkByRefName])
                    ? resource[linkByRefName].some(ref => ref === item.ref)
                    : resource[linkByRefName] === item.ref;
                }

                // indirect relationships
                if (relationshipDescriptor.type === 'indirect' && item[linkByRefName]) {
                  if (through && filterName) {
                    const {
                      fieldNamePath: throughFieldNamePath
                    } = this.resourcefulEndpoint.resourceful.relationships[
                      through
                    ];
                    let throughFields = resource[throughFieldNamePath] || [];
                    throughFields = Array.isArray(throughFields)
                      ? throughFields
                      : [throughFields];
                  ...
                  return Array.isArray(item[linkByRefName])
                    ? item[linkByRefName].some(ref => ref === resource.ref)
                    : item[linkByRefName] === resource.ref;
                }
                return true;
              });
            });
