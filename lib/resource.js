...
class Resource {
  ...
  toJSON() {
    ...
    if (this.getResourceFields()) {
      ...
      Object.keys(this.getResourceFields()).forEach((key) => {
        json[key] = this.hasOwnProperty(key) ? this[key] : this.getResourceFields()[key].default;
      });
    } else {
      json = this.rawData;
    }
  ...
  save() {
    if (this.resourcefulEndpoint) {
      ...
      return this.saveIndirectLinks().then(() => this.flush()).then(clearLinks);
    }
  ...
  validateField(field) {
      ...
      if (this[field] === undefined || this[field] === null) {
        ...
        if ('pattern' in typeInfo) {
          ...
          Object.keys(this[field]).forEach((key) => {
            if (!(re.test(key))) {
              message += `${field}.${key} does not match ${typeInfo.keys.pattern}. `;
    ...
    return { code, field, message, typeInfo: typeInformation, valid };
  }
  ...
  getRelationshipFor(resource) {
    ...
    return this.resourcefulEndpoint.relationshipFor(resource.resourcefulEndpoint.pluralName);
  }
  ...
  linkResource(resource, as) {
    ...
    if (relationship.type === 'direct') {
      ...
      this.linkRefToResource(this.ref, resource.getRelationshipFor(as || this), resource);
  ...
  hasLinked(relationship) {
    return this.linked && this.linked[relationship] && this.linked[relationship].length > 0;
  }
  ...
  get trailers() {
    return this.videos.filter((item) => {
      ...
      if (tags) {
        return item.tags.includes('trailerondemand') ||
    ...
    return this.images.find((item) => {
      ...
      if (tags) {
        return item.tags.includes('portrait') ||
               item.tags.includes('usage:boxart');
      }
  ...
  /* istanbul ignore next */
  primaryStill() {
    return this.images.find(item => item.tags && (item.tags.includes('usage:still') ||
                                    item.tags.includes('landscape')));
  }

  /* istanbul ignore next */
  trailer() {
    return this.trailers.find(item => item.tags && item.tags.includes('console:primary')) ||
           this.trailers[0];
  }
