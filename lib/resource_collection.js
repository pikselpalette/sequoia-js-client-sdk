...
class ResourceCollection {
  ...
  explode(size) {
    const batchSize =
      size !== undefined ? size : this.resourcefulEndpoint.batchSize;
  ...
  save(batchSize) {
    if (this.resourcefulEndpoint) {
      return Promise.all(
        this.explode(batchSize).map(c =>
          this.resourcefulEndpoint.store(c).catch(e => {
  ...
  destroy() {
    if (this.resourcefulEndpoint) {
      return Promise.all(
        this.explode().map(c =>
          this.resourcefulEndpoint.destroy(
